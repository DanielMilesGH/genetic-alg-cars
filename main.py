...
ALLOWED_AREA = 250
MIN_SPEED = 15  # cars can increase their speed
MAX_SPEED = 15  # can't be larger than car_size
TURN_SPEED = 30
...
FRAMES_UNTIL_COLLISION_CHECK = 1
...
ADVANCED_OBSTACLES = [pygame.Rect((300, 0, 30, 300)),
                      pygame.Rect((0, 500, 1700, 30)),
                      pygame.Rect((800, 200, 30, 500)),
                      pygame.Rect((1200, 0, 30, 250)),
                      pygame.Rect((1500, 200, 300, 30))]
...
async def main():
    ...
    while True:
        ...
        while (genLoop) and len(aliveList) > 0:
            ...
            for aliveCar in aliveList:
                if not aliveCar.alive:
                    continue
                aliveCar.quadrantCheck(X_WIDTH, Y_WIDTH, X_WIDTH / 8)
                # print(i.nnet)
                # print(len(i.quadrants))
                if not (aliveCar.move(bounds = (0, 0, X_WIDTH - CAR_SIZE, Y_WIDTH - CAR_SIZE), minSpeed=MIN_SPEED)):

                    # aliveList.remove(aliveCar)
                    # deceasedList.append(aliveCar)
                    # don't need to do above aliveList and deceasedList updates, can just set alive=false
                    aliveCar.alive = False
                    # pygame.display.set_caption('FPS: ' + str(clock.get_fps()) + ' NumAlive:' + str(len(aliveList)))

                # we can check this every FRAMES_UNTIL_COLLISION_CHECK
                if (frameCount % FRAMES_UNTIL_COLLISION_CHECK) == 0:
                    # TODO i would like collision checking to be part of the car class
                    for r in NOT_ALLOWED_RECTS:
                        if pygame.Rect.colliderect(r, pygame.Rect(aliveCar.x, aliveCar.y, 1, 1)):
                            # TODO UPDATE FITNESS
                            # aliveCar.fitness=0
                            try:
                                # aliveList.remove(aliveCar)
                        ...
                        #     'FPS: ' + str(clock.get_fps()) + ' NumAlive:' + str(len(aliveList)))

                aliveCar.checkpointCheck(CHECKPOINTS, CAR_SIZE)
                aliveCar.fitnessUpdate()
                ...
                if frameCount > FIRST_CHECK_FRAMES and len(aliveCar.quadrants) <= FIRST_CHECKPOINT_CHECK:
                    try:
                        # aliveList.remove(aliveCar)
                        # deceasedList.append(aliveCar)
                        aliveCar.alive=False
                        ...
                        pass
                    # pygame.display.set_caption('FPS: ' + str(clock.get_fps()) + ' NumAlive:' + str(len(aliveList)))
                if aliveCar.getOutput(*VISION_PARAMS)[0] > 0.5:
                    aliveCar.speedUp(inc=2, maxSpeed=MAX_SPEED)

                if aliveCar.getOutput(*VISION_PARAMS)[1] > 0.5:
                    aliveCar.turn(-TURN_SPEED)

                elif aliveCar.getOutput(*VISION_PARAMS)[2] > 0.5:
                    aliveCar.turn(TURN_SPEED)

                # if (len(i.quadrants) > 15) and REGEN:
                if (len(aliveCar.checkpointsHit) == len(CHECKPOINTS)) and REGEN:
                    aliveCar.fitness = 99999
                    genLoop = False
                    break
            ...
            # print(frameCount)
        print(frameCount)
        bestFitness = frameCount
        # got out of gen loop, update generation

        # first put everyone in deceased UPDATE: no longer needed
        # for i in range(len(aliveList)):
        #     deceasedList.append(aliveList.pop())
        for i in aliveList:
            i.fitnessUpdate()
        aliveList.sort(reverse=True)
        # print("list of best:", aliveList[0].checkpointsHit)
        # print("fitness of best:",aliveList[0].fitness)

        FITTEST = aliveList[0]
        for i in aliveList:
            i.reset(copy.deepcopy(FITTEST.nnet), *AICAR_PARAMS[1:])  # reset, fitness is saved by the ordering of list

        # # print([i.fitness for i in aliveList])
        # # TODO, for now getting most fit of all time, instead of top prop
        # if aliveList[0] > BEST_FITNESS:
        #     fittest=copy.deepcopy(aliveList[0])

        # BEST_FITNESS = fittest.fitness 
        # fittest.reset()
        # fittest.selected=True
        # for c in aliveList[0:round(NUM_CARS*PERCENT_TO_BREED)]:
        #     c.reset()
        ...
        newGenList = [FITTEST]
        for c in aliveList[1:round(NUM_CARS * PERCENT_TO_BREED)]:
            c.nnet.modify_weights()
            # print(c)
            newGenList.append(c)
            # print(c.nnet.printStuff())
            # aliveList[-1].nnet.modify_weights()
        # print(BEST_FITNESS)
        for c in aliveList[round(NUM_CARS * PERCENT_TO_BREED):]:
            c.reset(copy.deepcopy(nnet.Nnet(NUM_INPUT, NUM_HIDDEN, NUM_OUTPUT)),
                    *AICAR_PARAMS[1:])
            newGenList.append(c)

        aliveList = newGenList[:]
        print("alive length: " + str(len(aliveList)))
        print("newGenList length: " + str(len(newGenList)))
...
def physicsCheck():
    ...
    while True:
        ...
        raceCar.getOutput(*VISION_PARAMS)
